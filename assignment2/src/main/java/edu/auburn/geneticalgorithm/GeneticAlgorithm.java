...
public class GeneticAlgorithm<T, V extends Comparable<V>> {
	...
	GeneticAlgorithm(int populationSize, FitnessEvaluator<T, V> fitnessEvaluator, Crossover<T> crossover) {
		this.populationSize  = populationSize;
		this.fitnessEvaluator = fitnessEvaluator;
		this.crossover = crossover;
	}
	
	FitnessRecord<T,V> searchForBestIndividual(GeneticAlgorithmStrategy<T,V> strategy, int generations) {
		
		List<T> population = strategy.getRandomPopulation(populationSize);
		List<FitnessRecord<T, V>> fitnessRecords = strategy.getFitnessRecords(population, fitnessEvaluator);
		
		for(int i = 0; i < generations; i++) {
			
			List<FitnessRecord<T,V>> selection = strategy.selectIndividualsForReproduction(fitnessRecords);
